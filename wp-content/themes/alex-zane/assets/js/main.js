...
jQuery(function () {
    ...
    if (typeof pageCalculations !== 'function') { 
      ...
      var winW, winH,winS, pageCalculations;
    ...
    if ($.exists('.fullpage.jsScrollOff')) {
        /* Custom scrolling disabled */
        $('.jsScrollOff').fullpage({
            menu: '#menu',
            autoScrolling: false,
            lockAnchors: false,
            fitToSection: false,
    ...
    if ($.exists('.masonry')) {
        ...
        $container.imagesLoaded( function() {
            $container.masonry({
                itemSelector: '.grid-item, .blog-item,.timeline',
                normalScrollElements: '.mobile-menu-overlay',
                percentPosition: true
            });
            function animate() {
                $('.row img, .post-info, .image_thumbnail').each(function(i) {
                    (function(self, j) {
                        setTimeout(function() {
    ...
    $('.mobile-menu-content .menu-item-has-children > a').on('click', function(){
        ...
        var initPhotoSwipeFromDOM = function(gallerySelector) {
            ...
            var openPhotoSwipe = function(index, galleryElement, disableAnimation, fromURL) {
                ...
                for (var j = items.length - 1; j >= 0; j--) {
                    if ( items[j].el.children[0].getAttribute('data-iframe') ) {
                        items[j].html = '<div class="pswp__iframe"><iframe  width="600" height="400" src="' + items[j].el.children[0].getAttribute('data-iframe') + '"></div>';
                        items[j].src = '';
                        items[j].msrc = '';
                    }
                }
                ...
                gallery.listen('gettingData', function(index, item) {
                    ...
                    if( item.el.children[0].getAttribute('data-iframe') ) {
                        item.html = '<div class="pswp__iframe"><iframe  width="600" height="400" src="' + item.el.children[0].getAttribute('data-iframe') + '"></div>';
                        jQuery(".pswp__iframe").fitVids();
                    } 
                ...
                gallery.listen('close', function() { 
                    jQuery(".pswp__iframe iframe").attr('src','');
                });
            ...
            submitHandler: function (form) {
             $.ajax({
                 ...
                 success: function (data) {
                    if (data == "Email sent!");
                    $('input, textarea').val('');
                    $('button').html('Email sent <i class="fa fa-envelope-o"></i>');
                    setTimeout(function(){
    ...
    // FOR WP
    $('.alex-zane-like-it').on('click', function() {
      ...
      $.ajax({
       ...
       url: url,
       data: ({
        action: 'alex_zane_post_like',
        id: id,
        size: big_size,
       }),
       success: function(msg) {
        console.log(big_size);
        if (big_size) {
            $this.next('.like-count').html( msg );
        } else {
            $this.html( msg );
        }
    ...
    function alex_zane_add_img_bg( img_sel, parent_sel){
    ...
    alex_zane_add_img_bg('.alex-bg-image');



    var selectorEl = '.admin-bar .header .parallax';
    ...
    $('.comment-reply-link').append('<i class="fa fa-reply"></i>');

    $('.pager li a').addClass('btn');

    $('.pager li.prev a').prepend('<i class="fa fa-angle-left"></i>');
    $('.pager li.next a').append('<i class="fa fa-angle-right"></i>');

    $('.no_vc  iframe').each(function(i,el){
      var el = $(el);
      el.height((el.width()*0.7)-1);
    });
