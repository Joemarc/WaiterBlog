...
/**
 * Alex_Zane_Setup class
 */
class Alex_Zane_Setup {
	...
	public function admin_menus() {
		add_theme_page(esc_html__('Setup Theme and Import Demo','alex-zane'), esc_html__('Setup and Import Demo','alex-zane'), 'manage_options', $this->theme_name.'-setup', array($this,'setup_wizard') );
	}
	...
	public function setup_wizard() {
		...
		wp_localize_script( 'envato-setup', 'envato_setup_params', array(
			...
			'verify_text' => esc_html__('...verifying','alex-zane'),
		) );
		...
		$this->steps = array(
			'introduction' => array(
				'name'    =>  esc_html__( 'Introduction', 'alex-zane' ),
	...
	public function setup_wizard_footer() {
		?>
			...
			<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'alex-zane' ); ?></a>
			<?php endif; ?>
		</body>
		<?php
	...
	public function envato_setup_introduction() {
		if(isset($_REQUEST['export'])){
			...
			?>
			<h1><?php esc_html_e( 'Current Settings:','alex-zane'); ?></h1>
			<p><?php esc_html_e( 'Widget Positions:','alex-zane'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($widget_positions);?></textarea>
			<p><?php esc_html_e( 'Widget Options:','alex-zane'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($widget_options);?></textarea>
			<p><?php esc_html_e( 'Menu IDs:','alex-zane'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($menu_ids);?></textarea>
			<p><?php esc_html_e( 'Custom Options:','alex-zane'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($my_options);?></textarea>
			<p><?php esc_html_e( 'Copy these values into your PHP code when distributing/updating the theme.','alex-zane'); ?></p>
			<?php
		}else {
			?>
			<h1><?php esc_html_e( 'Welcome to the setup themes!', 'alex-zane' ); ?></h1>
			<p><?php esc_html_e( 'This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'alex-zane' ); ?></p>
			<p><?php esc_html_e( "No time right now? If you don't want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!", 'alex-zane' ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'alex-zane' ); ?></a>
				<a href="<?php echo esc_url( admin_url( 'edit.php' ) ); ?>"
				   class="button button-large"><?php esc_html_e( 'Not right now', 'alex-zane' ); ?></a>
			</p>
			<?php
		}
	}
	...
	public function envato_setup_default_plugins() {
		...
		?>
		<h1><?php esc_html_e( 'Default Plugins', 'alex-zane' ); ?></h1>
		<form method="post">
			...
			?>
				<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed:', 'alex-zane' ); ?></p>
				...
				echo '<p><strong>'.esc_html__('Good news! All plugins are already installed and up to date. Please continue.','alex-zane').'</strong></p>';
			} ?>

			<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'alex-zane'); ?></p>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e( 'Continue', 'alex-zane' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'alex-zane' ); ?></a>
	...
	public function ajax_plugins(){
		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])){
			wp_send_json_error(array('error'=>1,'message'=>esc_html__('No Slug Found','alex-zane')));
		}
		$json = array();
		...
		foreach($plugins['activate'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Activating Plugin','alex-zane'),
				);
				break;
			}
		}
		foreach($plugins['update'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Updating Plugin','alex-zane'),
				);
				break;
			}
		}
		foreach($plugins['install'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Installing Plugin','alex-zane'),
				);
				break;
			}
		}
		...
		if($json){
			...
			wp_send_json(array('done'=>1,'message'=>esc_html__('Success','alex-zane')));
		}
		exit;
	...
	private function _content_default_get(){
		...
		$content['pages'] = array(
			'title' => esc_html__( 'Pages', 'alex-zane' ),
			'description' => esc_html__( 'This will create default pages as seen in the demo.', 'alex-zane' ),
			'pending' => esc_html__( 'Pending.', 'alex-zane' ),
			'installing' => esc_html__( 'Installing Default Pages.', 'alex-zane' ),
			'success' => esc_html__( 'Success.', 'alex-zane' ),
			'install_callback' => array($this,'_content_install_pages'),
		);
		/*$content['widgets'] = array(
			'title' => esc_html__( 'Widgets', 'alex-zane' ),
			'description' => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'alex-zane' ),
			'pending' => esc_html__( 'Pending.', 'alex-zane' ),
			'installing' => esc_html__( 'Installing Default Widgets.', 'alex-zane' ),
			'success' => esc_html__( 'Success.', 'alex-zane' ),
			'install_callback' => array($this,'_content_install_widgets'),
		);*/
		$content['settings'] = array(
			'title' => esc_html__( 'Settings', 'alex-zane' ),
			'description' => esc_html__( 'Configure default settings.', 'alex-zane' ),
			'pending' => esc_html__( 'Pending.', 'alex-zane' ),
			'installing' => esc_html__( 'Installing Default Settings.', 'alex-zane' ),
			'success' => esc_html__( 'Success.', 'alex-zane' ),
			'install_callback' => array($this,'_content_install_settings'),
		);
		$content['customize'] = array(
			'title' => esc_html__( 'Customize options', 'alex-zane' ),
			'description' => esc_html__( 'Configure default customize settings', 'alex-zane' ),
			'pending' => esc_html__( 'Pending.', 'alex-zane' ),
			'installing' => esc_html__( 'Installing Default Customize Settings.', 'alex-zane' ),
			'success' => esc_html__( 'Success.', 'alex-zane' ),
			'install_callback' => array($this,'_content_install_customize'),
		);
		/*$content['products'] = array(
			'title' => esc_html__( 'Products', 'alex-zane' ),
			'description' => esc_html__( 'Insert default shop products and categories as seen in the demo.', 'alex-zane' ),
			'pending' => esc_html__( 'Pending.', 'alex-zane' ),
			'installing' => esc_html__( 'Installing Default Products.', 'alex-zane' ),
			'success' => esc_html__( 'Success.', 'alex-zane' ),
			'install_callback' => array($this,'_content_install_products'),
		);
		
		$content['menu'] = array(
			'title' => esc_html__( 'Menu', 'alex-zane' ),
			'description' => esc_html__( 'Insert default menu as seen in the demo.', 'alex-zane' ),
			'pending' => esc_html__( 'Pending.', 'alex-zane' ),
			'installing' => esc_html__( 'Installing Default Menu.', 'alex-zane' ),
			'success' => esc_html__( 'Success.', 'alex-zane' ),
	...
	public function envato_setup_default_content() {
		?>
		<h1><?php esc_html_e( 'Default Content', 'alex-zane' ); ?></h1>
		<form method="post">
			<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue.', 'alex-zane' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
			<table class="envato-setup-pages" cellspacing="0">
				<thead>
				<tr>
					<td class="check"> </td>
					<th class="item"><?php esc_html_e( 'Item', 'alex-zane' ); ?></th>
					<th class="description"><?php esc_html_e( 'Description', 'alex-zane' ); ?></th>
					<th class="status"><?php esc_html_e( 'Status', 'alex-zane' ); ?></th>
				</tr>
				</thead>
				<tbody>
			...
			<p><?php esc_html_e( 'Once inserted, this content can be managed from the WordPress admin dashboard.', 'alex-zane'); ?></p>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php esc_html_e( 'Continue', 'alex-zane' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'alex-zane' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		</form>
	...
	public function ajax_content(){
		...
		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])){
			wp_send_json_error(array('error'=>1,'message'=>esc_html__('No content Found','alex-zane')));
		}
		...
		if($json){
			...
			wp_send_json(array('error'=>1,'message'=>esc_html__('Error','alex-zane')));
		}
	...
	// settings codestar
	private function _content_install_codestar($file){
		$theme_options = $this->_get_json($file);
	...
	public function envato_setup_logo_design() {
		...
		?>
		<h1><?php esc_html_e( 'Logo &amp; Design', 'alex-zane' ); ?></h1>
		<form method="post">
			<p><?php esc_html_e('Please add your logo below. For best results, the logo should be a transparent PNG ( 190 by 35 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.', 'alex-zane' ); ?> </p>
			<table>
				<tr>
					<td>
						<div id="current-logo">
					...
					<td>
						<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'alex-zane' ); ?></a>
					</td>
				</tr>
			</table>


			<p><?php esc_html_e('Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'alex-zane' ); ?></p>
			...
			<p class="envato-setup-actions step">
				<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'alex-zane' ); ?>" name="save_step" />
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'alex-zane' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		</form>
	...
	public function envato_setup_customize(){
		...
		<h1><?php esc_html_e( 'Theme Customization', 'alex-zane' ); ?></h1>
		<p>
			<?php esc_html_e( 'Most changes to the website can be made through the Appearance > Customize menu from the WordPress dashboard.', 'alex-zane' ); ?>
		</p>
		<p><?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars. To control which "widget areas" appear, go to an individual page and look for the "Left/Right Column" menu. Here widgets can be chosen for display on the left or right of a page. More details in documentation.', 'alex-zane' ); ?></p>
		<p>
			<em>
			<?php esc_html_e( 'Advanced Users: If you are going to make changes to the theme source code please use a', 'alex-zane' );?>
				<a href="https://codex.wordpress.org/Child_Themes" target="_blank"><?php esc_html_e( 'Child Theme', 'alex-zane' );?></a>
			<?php echo sprintf(esc_html__( 'rather than modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without overwriting your source code changes. %s See %s in the main folder for a sample.', 'alex-zane' ), '<br/>', '<code>child-theme.zip</code>'); ?></em>
		</p>

		<p class="envato-setup-actions step">
			<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'alex-zane' ); ?></a>
		</p>
	...
	public function envato_setup_help_support(){
		?>
		<h1><?php esc_html_e( 'Help and Support', 'alex-zane' ); ?></h1>
		<p><?php esc_html_e( 'This theme comes with 6 months item support from purchase date (with the option to extend this period). This license allows you to use this theme on a single website. Please purchase an additional license to use this theme on another website.', 'alex-zane' ); ?></p>
		<ul>
			<li><?php esc_html_e( 'Availability of the author to answer questions', 'alex-zane' ); ?></li>
			<li><?php esc_html_e( 'Answering technical questions about item features', 'alex-zane' ); ?></li>
			<li><?php esc_html_e( 'Assistance with reported bugs and issues', 'alex-zane' ); ?></li>
			<li><?php esc_html_e( 'Help with bundled 3rd party plugins', 'alex-zane' ); ?></li>
		</ul>
		<?php echo sprintf( esc_html__( 'Item Support %s Include:', 'alex-zane' ), '<strong>DOES NOT</strong>' ); ?>
		<ul>
			<li><?php esc_html_e( 'Customization services', 'alex-zane' ); ?></li>
			<li><?php esc_html_e( 'Installation services', 'alex-zane' ); ?></li>
			<li><?php esc_html_e( 'Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)', 'alex-zane' ); ?></li>
		</ul>
		<?php 
		$support_policy = 'http://themeforest.net/page/item_support_policy';
		?>
		<p><?php esc_html_e( 'More details about item support can be found in the ThemeForest', 'alex-zane' ); ?> <a href="<?php echo esc_url( $support_policy, 'http'); ?>" target="_blank"><?php esc_html_e( 'Item Support Polity', 'alex-zane' ); ?></a>. </p>
		<p class="envato-setup-actions step">
			<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'alex-zane' ); ?></a>
			<?php wp_nonce_field( 'envato-setup' ); ?>
		</p>
		<?php
	}
	...
	public function envato_setup_ready() {
		...
		<h1><?php esc_html_e( 'Your Website is Ready!', 'alex-zane' ); ?></h1>

		<p><?php esc_html_e( 'Congratulations! The theme has been activated and your website is ready. Login to your WordPress dashboard to make changes and modify any of the default content to suit your needs.', 'alex-zane' ); ?></p>

		<div class="envato-setup-next-steps">
			<div class="envato-setup-next-steps-first">
				<h2><?php esc_html_e( 'Next Steps', 'alex-zane' ); ?></h2>
				<ul>
					<li class="setup-product"><a class="button button-primary button-large" target="_blank" href="<?php echo esc_url( home_url() ); ?>"><b><?php esc_html_e( 'VIEW YOUR NEW WEBSITE!', 'alex-zane' ); ?></b></a></li>
				</ul>
			</div>
			<div class="envato-setup-next-steps-last">
				<h2><?php esc_html_e( 'More Resources', 'alex-zane' ); ?></h2>
				<ul>
					<li class="documentation"><a href="#" target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'alex-zane' ); ?></a></li>
					<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'alex-zane' ); ?></a></li>
				</ul>
			</div>
		</div>
		<?php
