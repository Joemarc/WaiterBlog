			...
			$func_name = function() use( $carousel_id, $per_row, $autoplay, $hide_pagination_control, $hide_prev_next_buttons, $scroll_per_page, $wrap ) {
				...
				<script type="text/javascript">
					jQuery( document ).ready(function( $ ) {
						$(".slider-<?php echo esc_js( $carousel_id ); ?> ul").addClass('owl-carousel').owlCarousel({
...
if( ! function_exists( 'basel_images_gallery_shortcode' )) {
	function basel_images_gallery_shortcode($atts) {
		...
		$class .= ' view-' . $view;
		$class .= ' gallery-id-' . $carousel_id;

		if( $lightbox ) $class .= ' photoswipe-images';

		ob_start(); ?>
			<div class="basel-images-gallery<?php echo esc_attr( $class ); ?>">
			...
			<?php if ( $view == 'carousel' ): 
				...
				$func_name = function() use( $carousel_id, $slides_per_view, $autoplay, $hide_pagination_control, $hide_prev_next_buttons, $scroll_per_page, $wrap ) {
					...
					<script type="text/javascript">
						jQuery( document ).ready(function( $ ) {
							$(".gallery-id-<?php echo $carousel_id; ?> .gallery-images").addClass('owl-carousel').owlCarousel({
			            		rtl: $('body').hasClass('rtl'),
					            items: <?php echo esc_js( $items['desktop'] ); ?>, 
					            responsive: {
					            	979: {
	...
	function basel_shortcode_team_member($atts, $content = "") {
	    ...
	    $output .= '<div class="team-member text-' . esc_attr( $align ) . ' '.$el_class.'">';
            ...
            if ($linkedin != '' || $twitter != '' || $facebook != '' || $skype != '' || $google_plus != '' || $instagram != '') {
                $output .= '<div class="member-social"><ul class="social-icons icons-design-' . esc_attr( $style ) . ' icons-size-' . esc_attr( $size ) .'">';
	                if ($facebook != '') {
	                    $output .= '<li class="social-facebook"><a href="'.esc_url( $facebook ).'"><i class="fa fa-facebook"></i></a></li>';
	                }
	                if ($twitter != '') {
	                    $output .= '<li class="social-twitter"><a href="'.esc_url( $twitter ).'"><i class="fa fa-twitter"></i></a></li>';
	                }
	                if ($google_plus != '') {
	                    $output .= '<li class="social-google-plus"><a href="'.esc_url( $google_plus ).'"><i class="fa fa-google-plus"></i></a></li>';
	                }
	                if ($linkedin != '') {
	                    $output .= '<li class="social-linkedin"><a href="'.esc_url( $linkedin ).'"><i class="fa fa-linkedin"></i></a></li>';
	                }
	                if ($skype != '') {
	                    $output .= '<li class="social-skype"><a href="'.esc_url( $skype ).'"><i class="fa fa-skype"></i></a></li>';
	                }
	                if ($instagram != '') {
	                    $output .= '<li class="social-instagram"><a href="'.esc_url( $instagram ).'"><i class="fa fa-instagram"></i></a></li>';
	                }
                $output .= '</ul></div>';
            }
			...
			<?php 
				if( $layout == 'slider' ) {
					...
					$func_name = function() use( $carousel_id, $slides_per_view, $autoplay, $speed, $hide_pagination_control, $hide_prev_next_buttons, $scroll_per_page, $wrap ) {
						...
						<script type="text/javascript">
							jQuery( document ).ready(function( $ ) {
								$("#testimonials-<?php echo esc_js( $carousel_id ); ?>").addClass('owl-carousel').owlCarousel({
				            		rtl: $('body').hasClass('rtl'),
						            items: <?php echo esc_js( $items['desktop'] ); ?>, 
						            responsive: {
						            	979: {
	...
	function basel_shortcode_banners_carousel($atts = array(), $content = null) {
		...
		ob_start(); ?>
			<div class="banners-carousel-wrapper">
				<div id="banners-carousel-<?php echo ($carousel_id); ?>" class="banners-carousel<?php echo esc_attr( $class ); ?>" >
					<?php echo do_shortcode( $content ); ?>
				</div>
			</div>
			...
			<?php 
				...
				$func_name = function() use( $carousel_id, $slides_per_view, $autoplay, $speed, $hide_pagination_control, $hide_prev_next_buttons, $scroll_per_page, $wrap ) {
					...
					<script type="text/javascript">
						jQuery( document ).ready(function( $ ) {
							$("#banners-carousel-<?php echo esc_js( $carousel_id ); ?>").addClass('owl-carousel').owlCarousel({
			            		rtl: $('body').hasClass('rtl'),
					            items: <?php echo esc_js( $items['desktop'] ); ?>, 
					            responsive: {
					            	979: {
