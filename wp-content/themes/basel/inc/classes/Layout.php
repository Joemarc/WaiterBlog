...
class BASEL_Layout {
	...
	private function _set_sidebar_name() {
	    ...
	    $is_product_attr = false;

	    $queried_object = get_queried_object();

	    if( $queried_object && property_exists( $queried_object, 'taxonomy' ) ) {
	        $taxonomy = $queried_object->taxonomy;
	        $is_product_attr = ( substr($taxonomy, 0,3) == 'pa_' );
	    }

		if( basel_woocommerce_installed() && ( is_shop() || is_product_category() || is_product_tag() || $is_product_attr ) ) {
	...
	private function _set_sidebar_col_width() {
		...
		if( basel_woocommerce_installed() && ( is_shop() || is_product_category() || is_product_tag() ) ) {
	...
	public function get_main_container_class() {
		...
		if( basel_woocommerce_installed() && is_singular( 'product' ) ) {
